...
int Scene::SavePreview(const std::string& bitmapPath) {
	...
	if (!m_pPreviewBitmap) {
		m_pPreviewBitmap = create_bitmap_ex(8, c_ScenePreviewWidth, c_ScenePreviewHeight);
	}
	...
	for (SceneObject* sceneObject: m_PlacedObjects[PlacedObjectSets::PLACEONLOAD]) {
		if (const TerrainObject* terrainObject = dynamic_cast<TerrainObject*>(sceneObject)) {
			...
			if (pos.GetFloorIntX() < 0) {
				if (terrainObject->HasBGColorBitmap()) {
				...
				if (terrainObject->HasFGColorBitmap()) {
	...
	if (g_FrameMan.SaveBitmapToPNG(m_pPreviewBitmap, bitmapPath.c_str()) == 0) {
