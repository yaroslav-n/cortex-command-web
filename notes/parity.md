...
| in the menus, the text in a dropdown or a text box is centred vertically in its box and starts 2 px in from the left border, level with the open list's items | the user's request ([specs/menus.md](../specs/menus.md)). Upstream draws the line a fixed `HeightMargin` (0) below the box's top and `WidthMargin` (3) from its left edge, whatever the box's height: in the 18 and 20 px tall boxes (Select Activity, Saved Games' sort order and Save Name) it sat 2 px under the top border and 4 or 6 px above the bottom one, and 1 px from the left border. `MainMenuSubMenuSkin.ini` now sets `WidthMargin = 4` and `VAlignment = middle`, a key the port added to `GUITextPanel` that centres the font's line in the box (1 px lower in an 18 px box, 2 px in a 20 px one, unchanged in a 16 px one). The typing cursor follows `WidthMargin`; upstream drew it 2 px in, wherever the text was. The editors', buy menus' and console's skins are upstream's |
| in the buy menu, every faction's section starts open, and section headers stand on a darker shade of the list's colour | the user's request ([specs/menus.md](../specs/menus.md)). Upstream opened only the base module's items and the player's own tech's (`BuyMenuGUI::Clear`, `SetNativeTechModule`); every section now starts open, and a header still closes it. `CategoryChange` marks each module header (`GUIListPanel::Item::m_IsHeader`), and the list fills a marked row with its skin's `HeaderColorIndex`, a `[Listbox]` key the port added: 161 in `DefaultSkin.ini` against the green background's 146, and in the factions' buy menu skins 67 (brown, 70), 36 (orange, 71), 251 (dark grey, 94), 217 (sky blue, 209) and 250 (black, 251). A skin without the key, a mod's for instance, draws headers as upstream does |
| holding Left Alt does what holding the right mouse button does: it opens the pie menu, and letting go chooses the slice | the user's request ([specs/controls.md](../specs/controls.md)), in place of Ctrl, which player one's default controls use to crouch. Upstream binds Left Alt to nothing; see [input](input.md) |
